(() => {
  ...
  // extensions/checkout-timer/src/Checkout.js
  var Checkout_default = extension(
    ...
    (root, { shop, settings }) => {
      ...
      const calculateEndTime = (timer) => {
        ...
        const endTime = new Date(currentTime.getTime() + timer * 6e4);
        return endTime;
      };
      setTimeout(() => {
        console.log("==>", self, shop.storefrontUrl + "cart/clear");
        fetch(shop.storefrontUrl + "cart.js").then((response) => response.json()).then((data) => {
          console.log(data);
        });
      }, 3e3);
      ...
      var x = setInterval(() => {
        ...
        let minutes2 = Math.floor(distance % (1e3 * 60 * 60) / (1e3 * 60));
        let seconds = Math.floor(distance % (1e3 * 60) / 1e3);
        let timein = (days > 0 ? days + "d " : "") + (hours > 0 ? hours + "h " : "") + minutes2 + "m " + seconds + "s ";
        banner.updateProps({
